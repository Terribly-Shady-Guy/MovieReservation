...
namespace MovieReservation.RateLimiting
{
    ...
    {
        public Func<OnRejectedContext, CancellationToken, ValueTask>? OnRejected { get; } = async (rejectedContext, cancellationToken) =>
        {
            Endpoint? rateLimitedEndpoint = rejectedContext.HttpContext.GetEndpoint();
            EnableRateLimitingAttribute? rateLimiterPolicy = rateLimitedEndpoint?.Metadata.GetRequiredMetadata<EnableRateLimitingAttribute>();
            ...
            logger.AuthenticationRequestRateLimited(
                clientIpAddress,
                rateLimiterPolicy?.PolicyName ?? "Unknown policy",
